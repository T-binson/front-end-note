...
设计模式原则
	1.开闭原则：对扩展开放，修改关闭
	2.里氏转换原则：子类继承父类，可以单独运行
	3.依赖倒转原则：应用一个对象，如果这个对象有底层类型，直接引用底层
	4.接口隔离原则：每个接口应该是一种角色
	5.合成/聚合复用原则：新对象应使用一些已有的对象，使之成为新对象的一部分
...
工厂模式
	...
	代码：
		var gongchang = {};
		gongchang.yifu = function(arg) {
			this.gongren = 30;
		};
		gongchang.xiezi = function() {
			console.log('xiezi');
		}
		...
		gongchang.changzhang = function(param) {
			return new gongchang[param]();
		}
		var ins = gongchang.changzhang('yifu');
		...
		var XML = (new XHRHandler()).createFactory();
...
策略模式
	作用：
		...
		3.单独定义算法类，方便了单元测试
	注意：
		1.不仅可以封装算法，也可以封装几乎任何类型的规则，如果在不同时间应用不同的业务规则，就可以考虑应用策略模式来处理
	代码：

迭代器模式
	作用：
		1.为遍历不同的集合结构提供一个统一的接口，从而支持同样的算法在不同的集合结构上进行操作
		2.对于集合内部结果常常变化各异，如果不想暴露其内部的结构，同时让客户代码透明的访问其中的元素，这时可以考虑使用迭代器模式
	注意：
		1一般的迭代需要两个方法，hasNext和Next，以遍历所有对象
		2.遍历集合的同时更改其结构可能会导致问题
	代码：
		var arr = [1, 2, 3];
		var diedai = (function() {
...
建造者模式
	...
	代码：比如$.ajax({})方法
		...
		function Foreman(worker) {
			this.bulidhouse = function(worker) {
				worker.buildbedroom();
		...
		var foreman = new Foreman(worker);
		foreman.buildhouse();
		var builthouse = worker.utify();
...
代理模式
	...
	代码：
		...
		proxy.prototype.buying = function() {
			new Seller(new Buyer()).sell('20');
		}
...
命令模式
	...
	代码：
		var lian = {};
		lian.paobing = function() {};
		lian.bubing = function() {};
...
模板方法模式
	作用：
		...
		2.各子类中的公共行为应被提取出来并集中到一个公共父类中，避免代码重复，不同之处分离为新的操作，最后以一个钩子来替换这些代码
		3.控制子类扩展，模板方法只在特定点调用钩子函数
	注意：
		1.和策略模式不同，模板方法使用继承来改变算法的一部分，而策略模式使用委托来改变整个算法
	代码：
		function God() {};
		God.prototype.nose = function() {};
		God.prototype.eye = function() {};
		God.prototype.face = function() {};
		God.prototype.mouth = function() {};
		God.prototype.aihao = function() {};
		function xiaoming() {
			God.call(this);
		}
...
适配器模式
	...
	注意：
		1.与代理模式的区别，代理模式是不改变原接口，适配器模式是原接口不符合规范
	代码：
		function pp() {
			this.test = function() {};
		}
...
外观模式(开发者必备)
	作用：
		1.在设计初期，应该要有意识地将不同的两个层分离
		2.在开发阶段，子系统往往因为不断的重构而变得复杂，增加外观可以提供一个简单的接口，减少他们之间的依赖
		维护一个遗留的大型系统时，为系统开发一个外观类，可以设计粗糙和高度复杂的遗留代码提供比较清晰的接口，让新系统和这个外观类交互
	注意：
		1.连续使用时会产生一定的性能问题，因为每次调用都要检测功能的可用性
	代码：
		var fuhao = {}
